 ...
 *
 * Important Note:
 * This file has been modified by Refined Risk for some layout changes and improvements.
 * Be careful when replacing or upgrading this file with newer versions of Angular Loading; New version files
 * may need to be modified to in order to incorporate Refined Risk specific modifications from this file.
 */
(function (factory) {
    ...
    .value('loadingOptions', {
            ...
            spinnerContainerStyle: 'position:fixed; top:50%; left:50%;',
            spinnerTextContainerStyle: 'position:fixed; top:50%; left:50%;'
    })
                ...
                link: function (scope, element, attrs) {
                    ...
                    var update = function (newOptions, force) {
                        ...
                        if (options.spinner) {
                            ...
                            if (angular.isDefined(options.spinnerContainerStyle)) {
                                spinnerContainer.attr('style', options.spinnerContainerStyle);
                            }
                        ...
                        if (options.text) {
                            text = angular.element('<div></div>').addClass('dw-loading-text').text(options.text);

                            if (angular.isDefined(options.spinnerTextContainerStyle)) {
                                text.attr('style', options.spinnerTextContainerStyle);
                            }
