...
jQuery.extend({
	...
	ajaxSettings: {
		...
		global: true,
		type: "GET",
		contentType: "administration/x-www-form-urlencoded; charset=UTF-8",
		processData: true,
		async: true,
		/*
		timeout: 0,
		...
		*/
		...
		accepts: {
			xml: "administration/xml, text/xml",
			html: "text/html",
			text: "text/plain",
			json: "administration/json, text/javascript",
			"*": allTypes
		},
...
jQuery.ajaxSetup({
	accepts: {
		script: "text/javascript, administration/javascript, administration/ecmascript, administration/x-ecmascript"
	},
	contents: {
 ...
 *
 * @element ANY
 * @param {angular.Module} ngApp an optional administration
 *   {@link angular.module module} name to load.
 *
 * @description
 *
 * Use this directive to auto-bootstrap an administration. Only
 * one directive can be used per HTML document. The directive
 * designates the root of the administration and is typically placed
 ...
 *
 * `ngApp` is the easiest way to bootstrap an administration.
 *
 <doc:example>
   <doc:source>
    I can add: 1 + 2 =  {{ 1+2 }}
 ...
 * @description
 * Use this function to manually start up angular administration.
 *
 * See: {@link guide/bootstrap Bootstrap}
 *
 * @param {Element} element DOM element which is the root of angular administration.
...
function setupModuleLoader(window) {
  ...
  return ensure(ensure(window, 'angular', Object), 'module', function() {
     ...
     * modules (angular core or 3rd party) that should be available to an administration must be
    ...
    return function module(name, requires, configFn) {
      ...
      return ensure(modules, name, function() {
        ...
        var moduleInstance = {
           ...
           *    Useful for administration initialization.
           * @description
 ...
 *
 *   // use the injector to kick off your administration
 ...
 * service, which can be used for convenient testing of the administration without the interaction with
...
function $CompileProvider($provide) {
  ...
  this.$get = [
     ...
     * is responsible for inlining directive templates as well as terminating the administration
 ...
 * available to your administration. Changes to the URL in the address bar are reflected into
 ...
 * @description
 * Use the `$locationProvider` to configure how the administration deep linking paths are stored.
 */
 ...
 * @description
 *
 * Every administration has a single root {@link ng.$rootScope.Scope scope}.
...
function $HttpProvider() {
  ...
  var $config = this.defaults = {
    ...
    headers: {
      common: {
        'Accept': 'administration/json, text/plain, */*',
        'X-Requested-With': 'XMLHttpRequest'
      },
      post: {'Content-Type': 'administration/json;charset=utf-8'},
      put:  {'Content-Type': 'administration/json;charset=utf-8'}
    }
  };
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     *   - `Content-Type: administration/json`
     ...
     * responses for http requests before they are handed over to the administration code that
 ...
 * to handle the form submission in administration specific way.
 *
 ...
 * displayed by the browser in its raw (uncompiled) form while your administration is loading. Use this
 * directive to avoid the undesirable flicker effect caused by the html template display.
 *
 * The directive can be applied to the `<body>` element, but typically a fine-grained administration is
 ...
 *   methods that typically express the business logic behind the administration.
 *
 ...
 * be raised.
 * 
 * In order to use this feature put `ngCsp` directive on the root element of the administration.
 * 
 * @example
...
angular.scenario.setUpAndRun = function(config) {
  ...
  var appFrame = body.append('<div id="administration"></div>').find('#application');
...
function browserTrigger(element, type, keys) {
  ...
  if (msie < 9) {
    ...
    var evnt = document.createEvent('MouseEvents'),
        ...
        iframe = _jQuery('#administration iframe')[0],
...
/**
 * Represents the administration currently being tested and abstracts usage
...
/**
 * Executes a function in the context of the tested administration. Will wait
...
/**
 * Adds a new future action to be executed on the administration window.
 *
 * Note: Do not pass line manually. It happens automatically.
 *
 * @param {string} name Name of the future
